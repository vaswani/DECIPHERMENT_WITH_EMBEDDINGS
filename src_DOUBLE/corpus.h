...
using namespace std;
class Dict {
 typedef boost::unordered_map<std::string, unsigned, boost::hash<std::string> > Map;
 public:
  Dict() : b0_("<bad0>") {
    words_.reserve(1000);
  }
  ...
  inline void ConvertWhitespaceDelimitedLine(const std::string& line,
    ...
    while(cur < line.size()) {
      if (is_ws(line[cur++])) {
        if (state == 0) continue;
        out->push_back(Convert(line.substr(last, cur - last - 1)));
        out_words->push_back(line.substr(last, cur-last-1));
        state = 0;
      } else {
        if (state == 1) continue;
        last = cur - 1;
    ...
    if (state == 1) {
      out->push_back(Convert(line.substr(last, cur - last)));
      out_words->push_back(line.substr(last, cur - last));
    }
  }
