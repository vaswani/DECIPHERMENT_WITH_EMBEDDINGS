    ...
    int output_embedding_dimension)
{
    ...
    third_hidden_linear.resize(output_embedding_dimension, output_embedding_dimension);
    third_hidden_activation.resize(output_embedding_dimension);
    ...
    double adagrad_epsilon)
{
    ...
    second_hidden_linear.initialize(init_engine,
        ...
        parameter_update,
        adagrad_epsilon);
    third_hidden_linear.initialize(init_engine,
        init_normal,
        init_range,
        parameter_update,
        adagrad_epsilon);
...
void model::read(const string &filename, vector<string> &input_words, vector<string> &output_words)
{
	...
	else if (line == "\\hidden_weights 3")
	    third_hidden_linear.read_weights(file);
	else if (line == "\\hidden_biases 3")
	    third_hidden_linear.read_biases (file);
...
void model::write(const string &filename, const vector<string> *input_pwords, const vector<string> *output_pwords)
{
    ...
    file << "\\hidden_weights 3" << endl;
    third_hidden_linear.write_weights(file);
    file << endl;

    file << "\\hidden_biases 3" << endl;
    third_hidden_linear.write_biases(file);
    file << endl;
