...
public:
    ...
    Node<Linear_layer> third_hidden_linear_node;
    Node<Activation_function> third_hidden_activation_node;
...
public:
    ...
    propagator (model &nn, int minibatch_size)
      ...
      third_hidden_linear_node(&nn.third_hidden_linear, minibatch_size),
      third_hidden_activation_node(&nn.third_hidden_activation, minibatch_size),
      output_layer_node(&nn.output_layer, minibatch_size),
      minibatch_size(minibatch_size)
    {
      cerr<<"We are in the constructor for propagator"<<endl;
    }
    ...
    void resize(int minibatch_size) {
      ...
      third_hidden_linear_node.resize(minibatch_size);
      third_hidden_activation_node.resize(minibatch_size);
  ...
  if (parameter_update == "SGD") {
    ...
    {
        //cerr<<"Fpropping for SINGLE model "<<pnn<<endl;
        //getchar();
	...
	#ifdef TRIPLE
	third_hidden_linear_node.param->fProp(second_hidden_activation_node.fProp_matrix,
					       third_hidden_linear_node.fProp_matrix);
	third_hidden_activation_node.param->fProp(third_hidden_linear_node.fProp_matrix,
						   third_hidden_activation_node.fProp_matrix);
	#endif
...
private:
    ...
    void bPropRest(const MatrixBase<DerivedIn> &data,
       ...
       double decay) 
    {
       //cerr<<"Bpropping for SINGLE model "<<pnn<<endl;
	// Second hidden layer
  ...
  #ifdef TRIPLE
   start_timer(9);
   third_hidden_activation_node.param->bProp(output_layer_node.bProp_matrix,
                                           third_hidden_activation_node.bProp_matrix,
                                           third_hidden_linear_node.fProp_matrix,
                                           third_hidden_activation_node.fProp_matrix);


    third_hidden_linear_node.param->bProp(third_hidden_activation_node.bProp_matrix,
					       third_hidden_linear_node.bProp_matrix);
    stop_timer(9);
   start_timer(10);
   second_hidden_activation_node.param->bProp(third_hidden_linear_node.bProp_matrix,
  ...
  if (parameter_update == "SGD") {
    #ifdef TRIPLE
    start_timer(12);
    third_hidden_linear_node.param->computeGradient(third_hidden_activation_node.bProp_matrix,
                 second_hidden_activation_node.fProp_matrix,
                 learning_rate,
                 momentum,
                 L2_reg);
    ...
    #ifdef TRIPLE
    start_timer(12);
    third_hidden_linear_node.param->computeGradientAdagrad(third_hidden_activation_node.bProp_matrix,
                 second_hidden_activation_node.fProp_matrix,
                 learning_rate,
                 L2_reg);
    stop_timer(12);
    ...
    #ifdef TRIPLE
    start_timer(12);
    third_hidden_linear_node.param->computeGradientAdadelta(third_hidden_activation_node.bProp_matrix,
                 second_hidden_activation_node.fProp_matrix,
                 1.0/current_minibatch_size,
                 L2_reg,
                 conditioning_constant,
