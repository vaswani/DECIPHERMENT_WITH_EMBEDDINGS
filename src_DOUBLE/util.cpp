...
namespace nplm
{
// TAKES THE COMPOUND WORD AND RETURNS THE CONSTITUENT WORDS
// cat|sat|on -> [cat,sat,on]
void getTuple(const string& context_word,vector<string> *words) {
  size_t cur = 0;
  size_t last = 0;
  int state = 0;
  words->clear();
  //PIPE IS THE WORD SEPARATOR
  while(cur < context_word.size()) {
    if (context_word[cur++] == '|') {
      if (state == 0) continue;
      words->push_back(context_word.substr(last, cur - last - 1));
      state = 0;
    } else {
      if (state == 1) continue;
      last = cur - 1;
  ...
  if (state == 1)
    words->push_back(context_word.substr(last, cur - last));

}

/*
void getWordToInt(boost::unordered_map<std::string,unsigned> &word_to_int,
    const std::string filename) {
  vector<string> words;
  readWordsFile(filename,words);
  //now go over all the words and add them to the unordered map
  for (unsigned i =0; i<words.size(); i++){
    word_to_int.insert(make_pair(words[i],i));
    cerr<<"Added pair "<<words[i]<<" and "<<i<<" to the map"<<std::endl;
  }
}
*/
...
void getWordToInt(boost::unordered_map<std::string,unsigned> &word_to_int,
  ...
  while (getline(TRAININ, line) && line != "")
  {
    ...
    if (words.size() != 2)
    {
        cerr << "Error: word to int file must have only two entries per line" << endl;
        exit(-1);
    }
    int word_id = boost::lexical_cast<int>(words[1]);
    if (word_id > last_vocab_id) {
      last_vocab_id = word_id;
    }
    word_to_int.insert(make_pair(words[0],word_id));
    //cerr<<"Added pair "<<words[0]<<" and "<<words[1]<<" to the map"<<std::endl;
  }
}
